...
#include UE_INLINE_GENERATED_CPP_BY_NAME(XWorldWeaponInstance)

class UAnimInstance;
struct FGameplayTagContainer;

UXWorldWeaponInstance::UXWorldWeaponInstance(const FObjectInitializer& ObjectInitializer)
...
void UXWorldWeaponInstance::OnEquipped()
{
	Super::OnEquipped();
...
void UXWorldWeaponInstance::OnUnequipped()
{
	Super::OnUnequipped();
...
void UXWorldWeaponInstance::UpdateFiringTime()
{
	UWorld* World = GetWorld();
	check(World);
...
float UXWorldWeaponInstance::GetTimeSinceLastInteractedWith() const
{
	UWorld* World = GetWorld();
	check(World);
...
TSubclassOf<UAnimInstance> UXWorldWeaponInstance::PickBestAnimLayer(bool bEquipped, const FGameplayTagContainer& CosmeticTags) const
{
	const FLyraAnimLayerSelectionSet& SetToQuery = (bEquipped ? EquippedAnimSet : UneuippedAnimSet);
	return SetToQuery.SelectBestLayer(CosmeticTags);
}

const FPlatformUserId UXWorldWeaponInstance::GetOwningUserId() const
{
...
void UXWorldWeaponInstance::ApplyDeviceProperties()
{
...
void UXWorldWeaponInstance::RemoveDeviceProperties()
{
	const FPlatformUserId UserId = GetOwningUserId();
...
void UXWorldWeaponInstance::OnDeathStarted(AActor* OwningActor)
{
